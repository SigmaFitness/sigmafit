...
export const isInstanceDataValid = async (workoutType: string, data: SessionInstanceAddOrModifyBlockRequest['sets_data']) => {
    // type

    if (workoutType !== workout_type.WEIGHT_AND_REPS){
        data.forEach(values => {
            if(values.values.length >1) throw { "message": `Drop sets are only available for ${workout_type.WEIGHT_AND_REPS} type` }
        })
    }
