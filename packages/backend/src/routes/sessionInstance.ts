...
import { SessionInstanceAddOrModifyBlockRequest, SessionInstanceAddOrModifyBlockResponse, SessionInstanceAllActiveResponse, SessionInstanceEndResponse, SessionInstanceStartResponse, SessionInstanceStateResponse } from '@sigmafit/commons';
const router = Router()
...
router.post('/start/', isAuthenticated, async (req, res) => {
    try {
        ...
        const response: SessionInstanceStartResponse={...newSessionInstanceData}
        res.send(response)
    } catch (err) {
...
router.post('/addOrModifyBlock/', isAuthenticated, async (req, res) => {
    try {
        ...
        if (!blockData) throw { message: "check the workout_schema_id or the superset_workout_schema_id" }

        const validatedSetsData = await isInstanceDataValid(blockData.workout.category, data.sets_data);
        ...
        const response:SessionInstanceAddOrModifyBlockResponse = addedOrUpdatedBlock
        res.send(response)
    } catch (err) {
        return sendErrorResponse(res, err)
    }
