...
} from "../CreateNewOrEditWorkoutModal";
...
export type create_session_schema__superset_schema__without_order = Omit<
  create_session_schema__superset_schema,
  "order"
>;
export type create_session_schema__workout_schema__without_order = Omit<
  create_session_schema__workout_schema,
  "order"
>;
...
const SessionSchemaForm = ({
  ...
  useQuery<WorkoutListResponse, ErrorResponse>(
    "getAllWorkouts",
    getAllWorkouts,
    {
  ...
  const [initialWorkoutNameTypedValue, setInitialWorkoutNameTypedValue] =
    useState("");

  const setIsCreateNewWorkoutModalOpenAndPassInitValue = ({
    state,
    initialValue,
  }: {
    state: boolean;
            ...
            <div>
              <FieldArray
                ...
                render={(setsArrayHelpers) => (
                  <>
                    <ReactSortable
                      list={values.schema_blocks}
                      setList={(newList) => {
                        setFieldValue("schema_blocks", newList);
                      }}
                      handle=".move-handle"
                    >
                      {values.schema_blocks.map(
                        (schemaBlock, schemaBlockIndex) => {
                          if ("workout_id" in schemaBlock) {
                            const workoutInstance =
                              workoutIdToDataMap[schemaBlock.workout_id];
                            return (
                    ...
                    <div className="text-sm items-center gap-3 flex">
                      <div
                        onClick={() => {
                          const newWorkout: create_session_schema__workout_schema__without_order =
                            {
                      ...
                      <div
                        onClick={() => {
                          const newSupersetWorkout: create_session_schema__superset_schema__without_order =
                            {
                              id: Math.random().toString(),
                              name: "",
