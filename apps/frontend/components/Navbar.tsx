    ...
    return (
        <Menu>
            {({ open }) => (
                ...
                <div className={"navbar min-h-16 h-16 border-b z-50 transition-colors ease-in-out duration-300  " + (open ? 'bg-primary border-gray-700 top-0 left-0 w-full' : '')}>
                    ...
                    >
                        <Menu.Items as='div' className='z-50 flex px-6 text-center pt-5 flex-col text-white fixed left-0 bottom-0 w-screen bg-primary' style={{ height: 'calc(100vh - 64px)' }} static>
                            {!isLoading && !isError ?
                                <>
                                    <Menu.Item>
                                        <Link href='/dash'>
