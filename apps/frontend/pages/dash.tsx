...
const Dash = () => {
  ...
  const { data: sessionSchema, isLoading: isSessionSchemaLoading } = useQuery<
    SessionSchemaAllResponse,
    ErrorResponse
  >("getAllSessionSchemaOwnedByUser", getAllSessionSchemaOwnedByUser, {
    onSettled: (_, error) => {
      if (error) toast(error.message, { type: "error" });
    },
    retry: false,
  ...
  const { isLoading: waitingForMutateServerResponse, mutate } = useMutation<
    SessionInstanceStartResponse,
    ErrorResponse,
    string
  >(startANewSessionFromSchemaId);
        ...
        {sessionSchema ? (
          <>
            ...
            {sessionSchema.map((e, indx: number) => {
              ...
              const dateString = e.last_attempted_at
                ? `${date.toLocaleTimeString()}, ${date.toDateString()}`
                : "Never";
              return (
                <div key={indx}>
                  <div className="relative w-full p-4 flex flex-col gap-1 text-sm text-inherit py-2 px-4 rounded-md bg-black/[.09] my-3">
                    <div>
                      <span className="font-bold mr-1">Session Name:</span>
                      {e.name}
                    </div>
                    ...
                    <div className="flex justify-between mt-2">
                      <Link href={`/sessionSchema/${e.id}/view`}>
                        <div className="sm:absolute  btn sm:top-1 sm:right-1 btn-xs cursor-pointer select-none ">
                      ...
                      <button
                        disabled={
                          ...
                          waitingForMutateServerResponse
                        }
                        onClick={() => {
                          const res = confirm(
                            `Are you sure you want to start a new [${e.name}] session?`
                          );
                          if (res) {
                            // start a new session
                            mutate(e.id, {
                              onSettled(data, error, variables, context) {
                                if (error) {
                                  ...
                                  toast(
                                    "Redirecting to active session dash..",
                                    {
                                      type: "success",
                                      autoClose: 1000,
                                      onClose: () => {
                                        router.push(
                                          `/sessionInstance/${data.id}`
                                        );
                                      },
                                    }
                                  );
                        ...
                        className="sm:absolute  btn sm:bottom-1 sm:right-1 btn-xs  cursor-pointer select-none "
                      >
                        {" "}
                        Start new session
                      </button>
