...
export const addSessionSchemaPayloadValidator = yup.object().shape({
    ...
    schema_blocks: yup.array().required().min(1).of(yup.lazy((item) => {
        if (item.type === 'workout_schema_block') return workoutSchemaPayloadValidator;
        else if(item.type === 'superset_schema_block') return addSupersetSchemaPayloadValidator
        else throw {message: "Need an instance of workout or superset schema with type field as workout_schema_block | superset_schema_block"}
    }) as any)
});
